describe('Basic user flow for Website', () => {
  ...
  it('Make sure <product-item> elements are populated', async () => {
    ...
    for (let i = 0; i < prodItemsData.length; i++) {
      ...
      let currItem = prodItemsData[i];

      // Make sure title, price, and image are populated
      if (currItem.title.length == 0) { allArePopulated = false; }
      if (currItem.price.length == 0) { allArePopulated = false; }
      if (currItem.image.length == 0) { allArePopulated = false; }
    }
  ...
  it('Checking number of items in cart on screen', async () => {
    ...
    const cartCnt = await page.$('#cart-count');
    const textHandle = await cartCnt.getProperty('innerText');
    const text = await textHandle.jsonValue();
  ...
  it('Checking number of items in cart on screen after reload', async () => {
    ...
    // get all button texts from all product items
